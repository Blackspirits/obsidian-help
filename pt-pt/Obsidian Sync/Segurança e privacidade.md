...
For your safety, [[Introdução ao Obsidian Sync|Obsidian Sync]] encrypts your [[Cofres locais e remotos|remote vault]] and all communication with Obsidian's servers.
...
2. Disconnect the remote vault in each of your devices. This can be done by going to **[[Configurações]] → Sync → Pick remote vault → Disconnect**.
3. [[Configurar o Obsidian Sync#Create a new remote vault|Create a new remote vault]] on your primary device from the same Settings page. Optionally, you can delete the previous remote vault since you don't have the password for it anyway. (You may have to delete the previous remote vault if you are at the [[Perguntas frequentes#How many remote vaults can I have?|vault limit]])
...
1. Go to **[[Configurações]]** → **Sync** → **Copy Debug Info**.
